...
def validar_estado_brasileiro() -> Callable[[Any, Any], Any]:
    """
...
def validar_numero_inteiro(
    ...
    Example:
        _validar_idade = field_validator('idade')(
            validar_numero_inteiro('Idade', min_valor=18, max_valor=120)
        )
    """
    ...
    def validator(cls: Any, v: Any) -> Any:  # noqa: N805
        ...
        if numero_int < min_valor:
            raise ValueError(f"{nome_campo} deve ser maior ou igual a {min_valor}")

        if numero_int > max_valor:
            raise ValueError(f"{nome_campo} deve ser menor ou igual a {max_valor}")
